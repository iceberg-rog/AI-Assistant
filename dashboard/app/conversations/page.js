...
export default function ConversationsPage() {
  ...
  const [status, setStatus] = useState({ botOnline: false, bot: null, autoReply: false, processed: 0, lastActivity: null });
  ...
  const load = async () => {
    try {
      ...
      setUsers(d.users || []);
      setStatus({ botOnline: !!d.botOnline, bot: d.bot || null, autoReply: !!d.autoReply, processed: d.processed || 0, lastActivity: d.lastActivity || null });
    } catch {}
  };
  useEffect(() => {
    load();
  ...
  const totalEscalated = users.reduce((a, u) => a + u.escalatedCount, 0);
  ...
  return (
    <div>
      <PageHeader
        kicker="LIVE INBOX"
        title="مکالمات (زنده)"
        sub="هر ردیف یک کاربر. کلیک کن تا کلِ مکالمه و دکمه‌های هر دراف باز شه. هر ۴ ثانیه خودکار به‌روز می‌شود."
      />

      {/* live status bar */}
      <div className="rounded-xl border border-black/10 bg-black/[0.035] px-4 py-2.5 mb-4 flex items-center justify-between gap-3 text-[12px] flex-wrap">
        <div className="flex items-center gap-2 min-w-0">
          <span className={`h-2 w-2 rounded-full shrink-0 ${status.botOnline ? "bg-emerald-400 animate-pulse" : "bg-zinc-600"}`} />
          <span className="text-zinc-800 truncate">{status.botOnline ? (status.bot || "بات") : "بات آفلاین"}</span>
          <span className="text-zinc-500">·</span>
          <span className="text-zinc-500">{status.autoReply ? "ارسالِ خودکار" : "تأیید قبل از ارسال"}</span>
          <span className="text-emerald-600/70 text-[11px]">● زنده</span>
        </div>
        <div className="text-zinc-500 ltr flex items-center gap-2">
          <span>processed: {status.processed}</span>
          <span>·</span>
          <span>{timeHM(status.lastActivity)}</span>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        <Stat label="کاربران" value={users.length} accent="emerald" />
        <Stat label="درافِ معلق" value={totalPending} accent="sky" />
        <Stat label="ارجاع‌شده" value={totalEscalated} accent="amber" />
        <Stat label="پردازش‌شده" value={status.processed} />
      </div>
      ...
      {/* rows — one per user (ALL users with activity) */}
      <div className="rounded-xl border border-black/10 overflow-hidden">
        <div className="hidden md:flex items-center gap-3 px-4 py-2 text-[10px] text-zinc-500 bg-black/[0.035] border-b border-black/10">
          ...
          <div className="w-[110px] text-center">وضعیت</div>
        ...
        {visible.map((u) => (
          <button key={u.telegramId} onClick={() => open(u)} className="w-full flex items-center gap-3 px-4 py-3 text-right border-b border-black/[0.08] hover:bg-black/[0.04] transition">
            ...
            <div className="w-[110px] flex items-center justify-center gap-1.5">
              {u.pendingCount > 0
                ? <span className="rounded-full bg-sky-500/15 text-sky-700 ring-1 ring-sky-500/30 px-2.5 py-1 text-[12px] tabular-nums">{u.pendingCount}</span>
                : <span className="text-[10px] text-emerald-600/60">✓ پاسخ‌داده‌شده</span>}
              {u.escalatedCount > 0 && <span className="rounded-full bg-amber-500/15 text-amber-600 ring-1 ring-amber-500/30 px-2 py-1 text-[11px] tabular-nums">↗ {u.escalatedCount}</span>}
            </div>
        ...
        {visible.length === 0 && <div className="text-sm text-zinc-500 py-12 text-center">{q.trim() ? "چیزی پیدا نشد." : "هنوز مکالمه‌ای نیست."}</div>}
      </div>
